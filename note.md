...
7. 中断处理有很多问题，进行了一定的修正。

中断处理其实涉及到了相当多的问题：

1. 接收中断后需要操作 `mip` 寄存器，进入中断处理流程后应该首先取消 `mip` 的对应位。
2. 把输入的外部中断挂到 `DecodeUnit` 上可能被 `BRU` 或异常等冲刷，可能需要专门给中断开洞传输。
3. 如果直接在输入中断的时候立即处理，由于精确异常，当前正在执行的语句会被废掉，先处理中断然后重做，如果是多周期容易浪费时钟。

对于 2, 3 实际上是两种解决方案的利弊，最终为了好写选择了实现 3。
